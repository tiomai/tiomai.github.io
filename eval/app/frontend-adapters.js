(function(){
  ...
  const loadContexts=async()=>{
    ...
    if(!contextResult.error&&Array.isArray(contextResult.data))return contextResult.data.map(row=>({...row,capabilities:{canViewStudents:['teacher','administrator','school_admin'].includes(row.role),canViewResults:true,canAssign:['teacher','administrator','school_admin'].includes(row.role),canReview:['teacher','administrator','school_admin'].includes(row.role)}}));
    ...
    if(error)throw contextResult.error||error;
  ...
  const teacherStudents={async list({classId=null}={}){return (await callRpc('list_teacher_students',{requested_class_id:classId,requested_viewer_membership_id:await activeTeacherMembershipId()}))||{classId,items:[]}}};
