window.addEventListener('DOMContentLoaded',()=>{
  ...
  catalog.addEventListener('click',event=>{const button=event.target.closest('.pack button');if(!button)return;event.preventDefault();event.stopImmediatePropagation();const pack=button.closest('.pack').dataset.pack,activeMode=getMode();if(pack==='tw-gsat-115'){location.href=`student-taiwan-english-taking-mockup.html?year=115&mode=${activeMode}`;return}if(pack.startsWith('english-')){location.href=`student-assessment-taking-mockup.html?flow=${activeMode==='leisure'?'practice':'paper'}&pack=${pack}`;return}location.href=`student-math-practice-taking-mockup.html?mode=${activeMode}&pack=${pack}`},true);
  render();
});
