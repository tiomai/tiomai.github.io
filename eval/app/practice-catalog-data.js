window.addEventListener('DOMContentLoaded',()=>{
  ...
  catalog.addEventListener('click',event=>{const button=event.target.closest('.pack button');if(!button)return;event.preventDefault();event.stopImmediatePropagation();const pack=button.closest('.pack').dataset.pack,activeMode=getMode();if(pack.startsWith('english-')){location.href=`assessment-player/?flow=${activeMode==='leisure'?'practice':'paper'}&pack=${pack}`;return}location.href=`practice-player/?mode=${activeMode}&pack=${pack}`},true);
  render();
});
