window.addEventListener('DOMContentLoaded',async()=>{
  ...
  catalog.addEventListener('click',event=>{const button=event.target.closest('.pack button:not(.learning-reset-trigger)');if(!button)return;const card=button.closest('.pack'),pack=card.dataset.pack||card.dataset.practicePackId;if(!pack)return;event.preventDefault();event.stopImmediatePropagation();const activeMode=getMode(),subject=String(card.dataset.subject||'').toLowerCase(),title=card.querySelector('h3')?.textContent?.trim().toLowerCase()||'',key=pack.replace(/^pack-/,'');if(subject==='japanese'||key.startsWith('jlpt-')){location.href=`/eval/practice-player/?mode=${activeMode}&pack=${encodeURIComponent(key)}`;return}const english=subject==='english'||key.startsWith('english-')||['vocabulary','grammar','reading','listening challenge'].includes(title);if(english){const slug=key.startsWith('english-')?key:`english-${key}`;location.href=`/eval/assessment-player/?flow=${activeMode==='leisure'?'practice':'paper'}&pack=${encodeURIComponent(slug)}`;return}const mathAliases={trigonometry:'trigonometry-1'};location.href=`/eval/practice-player/?mode=${activeMode}&pack=${encodeURIComponent(mathAliases[key]||key)}`},true);
  render();
});
