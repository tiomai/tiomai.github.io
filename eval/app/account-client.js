(function(){if(!window.EXAI_CONTEXT_READY&&!document.querySelector('script[src*="app/context-bootstrap.js"]')){const script=document.createElement('script');script.src=location.pathname.includes('/eval/')?'/eval/app/context-bootstrap.js?v=20260912-7':'app/context-bootstrap.js?v=20260912-7';document.head.append(script)}})();
window.addEventListener('DOMContentLoaded', async () => {
  ...
  window.EXAI_CONTEXT_READY?.then(provider => {
    const sync = context => {
      if (context.status !== 'ready' || !context.user?.displayName) return;
      document.querySelectorAll('.identity b,[data-profile-name]').forEach(node => { node.textContent = context.user.displayName; });
    };
