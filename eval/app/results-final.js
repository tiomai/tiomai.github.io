window.addEventListener('DOMContentLoaded',()=>{
  ...
  const title=document.querySelector('.section-title');if(title)title.textContent='Assessment history';
  ...
  document.querySelector('#results')?.addEventListener('click',event=>{
    const trigger=event.target.closest('[data-id]');if(!trigger)return;const record=typeof data!=='undefined'?data.find(item=>String(item.id)===trigger.dataset.id):null,title=record?.title||'',target=/Math/.test(title)?'/eval/practice-player/?mode=assessment&pack=math-s4&review=result':`/eval/assessment-player/?review=result&pack=${/Listening/.test(title)?'english-listening':/DSE/.test(title)?'dse-reading':'english-reading'}`;
