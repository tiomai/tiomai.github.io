...
document.querySelectorAll('.pin-media').forEach(b=>b.addEventListener('click',()=>{modal.querySelector('img').src=b.dataset.src;modal.querySelector('img').alt=b.dataset.title;modal.querySelector('h2').textContent=b.dataset.title;const cardDrive=b.closest('.pin')?.querySelector('.pin-body a[href^=\"https://drive.google.com\"]')?.href; modal.querySelector('a').href=b.dataset.drive || cardDrive || 'https://drive.google.com/drive/folders/1O-DoyQILHphHDRthStbN0SGrFFkLFman';modal.querySelector('a').removeAttribute('download');modal.querySelector('a').textContent='Open in Google Drive ↗';modal.showModal();}));
