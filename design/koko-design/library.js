...
document.querySelectorAll('.pin-media').forEach(b=>b.addEventListener('click',()=>{modal.querySelector('img').src=b.dataset.src;modal.querySelector('img').alt=b.dataset.title;modal.querySelector('h2').textContent=b.dataset.title;modal.querySelector('a').href=b.dataset.drive || 'https://drive.google.com/drive/folders/1O-DoyQILHphHDRthStbN0SGrFFkLFman';modal.querySelector('a').removeAttribute('download');modal.querySelector('a').textContent='Open in Google Drive ↗';modal.showModal();}));
